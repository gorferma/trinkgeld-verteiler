...
export default function App() {
  ...
  <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Anteile in %, 0–100 je Person (Summe beliebig). Hinweis: Wenn Stammpersonal vorhanden ist, muss mindestens eine Person 100% (1,0) haben.</p>
        ...
        className="col-span-6 h-11 rounded-lg border px-3 bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700 focus-ring" />
              ...
              className={`w-full h-11 rounded-lg border pl-3 pr-12 lg:pr-20 bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700 focus-ring ${tooHigh ? 'border-red-300 focus:outline-none focus:ring-2 focus:ring-red-400' : ''}`} />
                          {tooHigh && (
                    ...
                    <span aria-hidden className="pointer-events-none absolute right-3 lg:right-10 top-1/2 -translate-y-1/2 text-gray-600">%</span>
              ...
              <table className="w-full text-sm table-fixed">
                <thead>
                  <tr className="text-left text-gray-600">
                    <th className="py-1 w-1/2 break-words">Name</th>
                    <th className="py-1 w-1/4">Betrag</th>
                    <th className="py-1 w-1/4">% Gruppe</th>
                  </tr>
                </thead>
                <tbody>
                  {results.staffRows.map(r => (
                    <tr key={r.id} className="border-t">
                      <td className="py-1 break-words">{r.name || '—'}</td>
